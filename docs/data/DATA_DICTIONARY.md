...
**`dollars_actions_by_naics4_category_fy.csv`** — Contract obligations and contract-action counts by fiscal year, four-digit NAICS industry, and socioeconomic category, FY2022–FY2026 (FY2026 partial). One row per `fiscal_year` × `naics4` × `category`. Columns: `fiscal_year` (2022–2026; 2026 partial), `naics4` (four-digit NAICS from the award's NAICS code; blank where the award carries no NAICS), `category`, `dollars` (net obligations, USD; de-obligations included as negatives), `actions` (contract-action count). Category values: `all`, `small_business`, `sdb`, `eight_a`, `wosb` (WOSB program flag), `edwosb`, `hubzone`, `sdvosb`, `vosb`, `woman_owned` (broader women-owned flag; not limited to the WOSB program — do not sum with `wosb`). Categories are non-exclusive; do not sum across categories. Built by a straight group-by of the FY2022–FY2026 base table with no thresholds, banding, or exclusions, using the same category definitions as the exposure build; per-fiscal-year totals for `all` and `small_business` reconcile to the published goaling-consistent fiscal-year summary.
